...
def get_price_from_akshare(ticker_symbol, spot_cache=None, etf_cache=None):
    ...
    try:
        ...
        if ticker_symbol.startswith('0'):
            # 优先尝试：开放式基金净值走势 (fund_open_fund_info_em)
            # 这个接口通常比 daily 更稳定，返回 'y' 列作为净值
            try:
                # print(f"   [Akshare] 尝试获取开放式基金净值走势: {ticker_symbol} ...")
                ...
                # print(f"   [Akshare] 开放式基金净值走势失败: {e}")
                pass

            # 其次尝试：开放式基金净值 (fund_open_fund_daily_em)
            try:
                # print(f"   [Akshare] 尝试获取开放式基金实时净值: {ticker_symbol} ...")
