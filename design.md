...
ETF_INDEX_MAPPING = {
    ...
    # 使用 stock_market_pe_lg('创业板') 获取创业板整体市场PE，比创业板50更准确反映创业板指的估值
...
- PE 百分位基于网页表格中的历史PE数据计算：统计低于当前PE的历史数据占比
- HSI 有约50年月度数据，HSCE 有约2年数据，HSTECH 暂无表格数据（2020年推出）
...
- **创业板 ETF**：使用乐咕乐股 `stock_market_pe_lg('创业板')` 获取创业板市场整体PE数据（月度数据，约15年历史）
