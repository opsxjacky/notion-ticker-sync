...
| `PEG` | number | 市盈增长比率 |
| `动作` | select | 交易动作（买入/卖出） |
| `成交价格` | number | 交易时的成交价格（手动填写） |
| `卖出后涨跌幅` | number | 卖出后价格涨跌幅 (%)，自动计算 |

#### 1.3 卖出后跟踪功能

当记录的「动作」字段包含"卖出"时，系统会自动计算「卖出后涨跌幅」：

**计算公式**：
```
卖出后涨跌幅 = (当前价格 - 成交价格) / 成交价格 × 100%
```
...
**使用方法**：
1. 在 Notion 中将「动作」设为包含"卖出"的选项
2. 填写「成交价格」为卖出时的实际价格
3. 运行脚本后，「卖出后涨跌幅」会自动更新
...
| `动作` | select | 交易动作（买入/卖出等） |
| `成交价格` | number | 交易时的成交价格 |
| `卖出后涨跌幅` | number | 卖出后价格变化百分比 |
